...
class LeadMahasiswaController extends Controller
{
    public function index()
    {
        $users =  User::orderBy('name', 'ASC')->filter(request(['search']))->paginate(10)->withQueryString();
       return view('lead.mahasiswa.index', compact('users'));
    }

    public function create()
    {
        return view('lead.mahasiswa.add');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('lead.mahasiswa')->with($notification);
    }

    public function edit($id)
    {
        $user = User::findOrFail($id);
        return view('lead.mahasiswa.edit', compact('user'));
    }
