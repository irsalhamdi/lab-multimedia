    ...
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('asistant.contact.update') }}" class="forms-sample">
                    @csrf
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('select[name="province_id"]').on('change', function(){
                ...
                if(province_id) {
                    $.ajax({
                        url: "{{  url('/asistant/get-regency/ajax') }}/"+province_id,
                        type:"GET",
                        dataType:"json",
            ...
            $('select[name="regency_id"]').on('change', function(){
                ...
                if(regency_id) {
                    $.ajax({
                        url: "{{  url('/asistant/get-district/ajax') }}/" + regency_id,
                        type:"GET",
                        dataType:"json",
                        success:function(data) {
            ...
            $('select[name="district_id"]').on('change', function(){
                ...
                if(district_id) {
                    $.ajax({
                        url: "{{  url('/asistant/get-village/ajax') }}/" + district_id,
                        type:"GET",
                        dataType:"json",
                        success:function(data) {
