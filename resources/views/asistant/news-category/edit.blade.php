...
@section('main')
    <div class="col-12 grid-margin stretch-card">
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('asistant.news-category.update', $category->id) }}" class="forms-sample">
                @csrf
                <div class="form-group">
                    <label>Kategori</label>
                    <input type="text" name="name" value="{{ $category->name }}" class="form-control" placeholder="Nama Kategori" required>
                </div>
                <a href="{{ route('asistant.news-category') }}" class="btn btn-light">Cancel</a>
