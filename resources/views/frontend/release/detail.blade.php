            ...
            <div class="col-lg-8 posts-list">
                <div class="single-post">
                    <div class="feature-img">
                        ...
                        <div class="container">
                            <p align="center">
                                <iframe type="application/pdf" src="{{ asset($release->file) }}" width="300" height="600"></iframe>
                            </p>
                        </div>
                        @else
                            <img class="img-fluid" src="{{ asset($release->file) }}">
                        @endif
                    </div>
                ...
                <div class="navigation-top">
                    <div class="d-sm-flex justify-content-between text-center">
                        <p class="like-info"><span class="align-middle"><i class="fa fa-clock-o"></i></span> {{ $release->created_at}}</p>
                ...
                <div class="comment-form">
                    ...
                    @if(Auth::guard('admin')->check())
                        <form method="POST" action="{{ route('admin.comment.release', $release->id) }}" class="form-contact comment_form" action="#" id="commentForm">
                    @elseif(Auth::guard('asistant')->check())
                        <form method="POST" action="{{ route('asistant.comment.release', $release->id) }}" class="form-contact comment_form" action="#" id="commentForm">
                    @elseif(Auth::guard('dosen')->check())
                        <form method="POST" action="{{ route('dosen.comment.release', $release->id) }}" class="form-contact comment_form" action="#" id="commentForm">
                    @elseif(Auth::guard('lead')->check())
                        <form method="POST" action="{{ route('lead.comment.release', $release->id) }}" class="form-contact comment_form" action="#" id="commentForm">
                    @elseif(Auth::guard('web')->check())
                        <form method="POST" action="{{ route('mahasiswa.comment.release', $release->id) }}" class="form-contact comment_form" action="#" id="commentForm">
                    @elseif (!Auth::user())
                        <form method="POST" action="{{ route('mahasiswa.comment.release', $release->id) }}" class="form-contact comment_form" action="#" id="commentForm">  
                    @endif
                        @csrf
